...
export class PlainClient {
	...
	async removeCustomerFromCustomerGroups(
		input: VariablesOf<
			typeof RemoveCustomerFromCustomerGroupsDocument
		>["input"],
	): SDKResult<null> {
		const res = await request(this.#ctx, {
		...
		return unwrapData(res, (q) => {
			return {
				customerGroupMemberships: q.customer
					...
					: [],
				pageInfo: q.customer
					? q.customer.customerGroupMemberships.pageInfo
					: null,
			};
		});
	}
	...
	async getCustomerTenantMemberships(
		...
		return unwrapData(res, (q) => {
			return {
				tenantMemberships: q.customer
					? q.customer.tenantMemberships.edges.map((e) => e.node)
					: [],
	...
	async createCustomerEvent(
		...
		return unwrapData(res, (q) =>
			nonNullable(q.createCustomerEvent.customerEvent),
		);
	}
	...
	async createWebhookTarget(
		...
		return unwrapData(res, (q) =>
			nonNullable(q.createWebhookTarget.webhookTarget),
		);
	}
	...
	async updateWebhookTarget(
		...
		return unwrapData(res, (q) =>
			nonNullable(q.updateWebhookTarget.webhookTarget),
		);
	}
	...
	async updateCustomerCompany(
		...
		return unwrapData(res, (res) =>
			nonNullable(res.updateCustomerCompany.customer),
		);
	}
	...
	async updateTenantTier(
		...
		return unwrapData(res, (q) =>
			nonNullable(q.updateTenantTier.tenantTierMembership),
		);
	}
	...
	async updateCompanyTier(
		...
		return unwrapData(res, (q) =>
			nonNullable(q.updateCompanyTier.companyTierMembership),
		);
	}
